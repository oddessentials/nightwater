...
**Rule (updated for final polish):** the highest tier caught arms the next answer. Correct → `base × mult` plus a fading quick-answer bonus of up to 50%; wrong → subtract 75% of `base × mult`, limited to the points available. Both outcomes consume the multiplier. See Quick Answer below for timing and presentation.
...
- `journey.ts` / `save.ts` / `panel.ts` — default old saves to score 0, multiplier 1, and elapsed answer time 0; validate all fields. Correct answers use the answered level and speed bonus, then consume the multiplier; wrong answers consume it and deduct points. The win card shows total score.
...
3. **Wrong-answer deductions — always on.** The final polishing instruction supersedes the earlier decision against deductions. A mistake costs 75% of the question's multiplied stake, capped at the current score. Display the actual possible loss before choosing and explain the deduction alongside the correct answer. No debt, separate mode, or opt-out.
...
## Quick Answer — final polish

The existing multiplied stake remains the minimum reward for a correct answer. A correct response earns up to 50% extra: full bonus for the first two seconds, then a continuous linear fade to zero at 15 seconds for stages 1–5, 25 seconds for stages 6–12, and 35 seconds for stages 13–21. Round the bonus to whole points. These are initial tuning values, not calibrated player-performance targets. Expiry neither submits an answer nor deducts points.

A wrong answer deducts 75% of the multiplied stake, capped at the score available. At a fixed stake, one fastest correct guess balances two wrong guesses before that floor; smaller penalties would make blind three-choice guessing profitable. The score floor deliberately forgives mistakes near zero. The existing stage progression, fresh question after a mistake, current, and route physics remain intact.

Start timing after the landing's panel and world signs have been presented. Destination signs show their route names during the approach, then reveal the question together with the panel. Selection by click, tap, or 1–3 captures the decision time, while the score still changes at the normal route event. Changing a choice replaces its timestamp; repeating it does nothing. Paddling cancels that selection and its timestamp while the original question clock continues. Swimming and drifting capture elapsed time at the pipe mouth, excluding the entry animation. Normal untouched drift reaches a mouth around 40 seconds, beyond all three bonus windows.

The clock uses monotonic elapsed time independent of physics steps, rendering rate, and ride speed. Pause freezes it and hides the panel and world question signs. Save the pending question's elapsed milliseconds periodically and at selection, cancellation, entry, pause, and page exit. Reload preserves the question, score, multiplier, and spent time; the replayed feeder does not count. A cancelled journey to an exit does not retain its provisional selection on reload. Old saves migrate without resetting progress. Restart clears timing; free riding never scores.

Keep feedback in the existing stake area and answer caption. Show the base stake and multiplier, the remaining Quick Answer points, the actual wrong-answer cost, and a thin fading meter. Reserve the layout's height and retain prompt and answer sizes. On short screens, scroll the question area while keeping the answer buttons and paddle reachable; each fresh question starts at the top. The pause panel also scrolls when needed. The successful result states the total and quick-answer contribution, with a brief score pulse and quiet two-note chime. Mistakes state the actual deduction and correct answer. Only the result is announced by the live caption; the moving meter is decorative. Pause help explains both percentages.

Validation includes scoring boundaries and all multipliers, recovery at zero, stage transitions and the final answer, original-clock cancellation, duplicate input, active and paused reloads, manual and idle exits, every ride speed, and a stalled-frame clock check. Run `npm test`, `npm run test:scoring`, `npm run test:catches`, `npm run test:browser`, `npm run test:rides`, `npm run test:lifecycle`, lint, formatting, and the production build. Review the existing long-math/figure screenshots on desktop and phone. Player testing is still needed to calibrate answer times, mistake rates, and recovery after high-multiplier losses.
