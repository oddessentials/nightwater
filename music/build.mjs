...
async function click(input, gain, output) {
  const [heard, made] = await Promise.all([
    levels(input, gain),
    levels(output, 0),
  ]);
...
async function encode({ title, source }) {
  ...
  assert.ok(
    Math.abs(after.loudness - target) <= 0.5,
    `${title} ${after.loudness} LUFS`,
  );
  assert.ok(after.peak <= ceiling, `${title} ${after.peak} dBTP`);
  assert.ok(
    Math.abs(after.duration - before.duration) <= 0.1,
    `${title} ${after.duration}s`,
  );
...
assert.equal(
  new Set(album.map((track) => slug(track.title))).size,
  album.length,
);
