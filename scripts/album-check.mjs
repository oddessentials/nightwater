...
const url = (index) => new URL(tracks[index % tracks.length].src, `${music}/`).href;
...
try {
  ...
  const album = () =>
    page.evaluate(() => {
      const { context, element, analyser } = window.__album;
  ...
  const context = (state) =>
    page.waitForFunction((state) => window.__album.context.state === state, state);
  const seek = (fromEnd) =>
    page.evaluate((fromEnd) => {
      const { element } = window.__album;
      element.currentTime = fromEnd ? element.duration - 1 : element.duration / 2;
    }, fromEnd);
  ...
  for (let index = 0; index < tracks.length; index++) {
    ...
    assert.ok(Math.abs(track.duration - tracks[index].duration) < 0.1, tracks[index].src);
    await seek(true);
    await playing(index + 1);
    console.log(`${index + 1}. ${tracks[index].title} -> ${tracks[(index + 1) % tracks.length].title}`);
  }
